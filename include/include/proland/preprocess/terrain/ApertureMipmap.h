...
public:
	...
	{
        std::list<FloatTile*>::iterator i = tileCacheOrder.begin();
...
private:
	...
	typedef std::map<int, std::list<FloatTile*>::iterator> Cache;

	Cache tileCache;

	std::list<FloatTile*> tileCacheOrder;
};
...
public:
	...
	DemTileCache(const std::string &name, int capacity = 20);
...
public:
    ...
    void build(const std::string &temp);

    void generate(const std::string &cache, const std::string &file);
