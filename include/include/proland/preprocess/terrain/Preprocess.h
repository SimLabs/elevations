...
private:
	...
	typedef std::map<std::pair<int, int>, std::list<Tile*>::iterator> Cache;

    int capacity;

	Cache tileCache;

	std::list<Tile*> tileCacheOrder;
...
PROLAND_API void preprocessDem(InputMap *src, int dstMinTileSize, int dstTileSize, int dstMaxLevel,
        const std::string &dstFolder, const std::string &tmpFolder, float residualScale);
 ...
 * @param tmpFolder where temporary files must be saved.
 */
PROLAND_API void preprocessSphericalAperture(const std::string &srcFolder, int minLevel, int maxLevel, int samples,
        const std::string &dstFolder, const std::string &tmpFolder);
...
PROLAND_API void preprocessOrtho(InputMap *src, int dstTileSize, int dstChannels, int dstMaxLevel,
        const std::string &dstFolder, const std::string &tmpFolder, float (*rgbToLinear)(float) = NULL, float (*linearToRgb)(float) = NULL);
