...
public:
    ...
    ColorMipmap(ColorFunction *colorf, int baseLevelSize, int tileSize, int border, int channels, float (*rgbToLinear)(float), float (*linearToRgb)(float), const std::string &cache);
    ...
    void generate(int rootLevel, int rootTx, int rootTy, bool dxt, bool jpg, int jpg_quality, const std::string &file);

    void generateResiduals(bool jpg, int jpg_quality, const std::string &in, const std::string &out);

    void reorderResiduals(const std::string &in, const std::string &out);
...
protected:
    ...
    std::map<int, int> constantTileIds;
