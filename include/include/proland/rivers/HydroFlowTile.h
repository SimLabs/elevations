...
public:
     ...
     */
    void addBanks(std::vector< ptr<HydroCurve> > &curves, float maxWidth);
...
private:
    ...
    public:
         ...
         */
        std::vector<int> edges[MAX_BANK_NUMBER];
         ...
         */
        std::set<CurveId> riverIds;
    };

    /**
     * Maps a CurveId to a River axis.
     */
    std::map<CurveId, int> rivers;
     ...
     */
    std::map<CurveId, std::vector<int> > riversToBanks;

    /**
     * The whole Curves list.
     */
    std::vector<ptr<HydroCurve> > banks;
     ...
     */
    void getLinkedEdges(vec2d &pos, DistCell *distCell, int riverId, std::set<int> &bankIds);
     ...
     */
    void getDistancesToBanks(vec2d &pos, DistCell *distCell, std::set<int> &bankIds, std::map<int, float> &distances);
     ...
     */
    void getPotential(vec2d &pos, std::map<int, float> &distances, float &potential, int &type);
