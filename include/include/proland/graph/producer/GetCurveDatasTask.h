...
public:
    ...
    virtual bool run()
    {
        ...
        std::set<TileCache::Tile::Id> tileIds;
        std::set<TileCache::Tile *> usedTiles;
        std::set<CurveId> usedDatas;
        ...
        if (target != NULL) {
            ...
            int n = 0;
            for (std::set<TileCache::Tile::Id>::iterator i = tileIds.begin(); i != tileIds.end(); i++) {
