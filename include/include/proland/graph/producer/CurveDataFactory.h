...
public:
     ...
     */
    void addUsedCurveDatas(int level, int tx, int ty, std::set<CurveId> curveDatas);
...
protected:
     ...
     */
    std::map<CurveId, CurveData*> curveDatas;

    /**
     * Reference count for each CurveData.
     */
    std::map<CurveData*, int> usedDataCount;

    /**
     * Maps every tile to the CurveDatas it uses.
     */
    std::map<TileCache::Tile::Id, std::set<CurveId> > usedDatas;
};
