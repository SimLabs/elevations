...
public:
    ...
    public:
         ...
         */
        GraphCache(GraphPtr root,  std::string graphName, ptr<ResourceManager> manager,
    ...
    private:
         ...
         */
        std::map<TileCache::Tile::Id, GraphPtr> graphs;
    };
     ...
     */
    GraphProducer(std::string name, ptr<TileCache> cache,
            ptr<GraphCache> precomputedGraphs, std::set<int> precomputedLevels,
...
protected:
     ...
     * See #GraphProducer.
     */
    void init(std::string name, ptr<TileCache> cache, ptr<GraphCache> precomputedGraphs,
        std::set<int> precomputedLevels, bool doFlatten = false, float flatnessFactor = 0.5f,
        bool storeParents = false, int maxNodes = 0);
...
private:
     ...
     */
    void updateFlattenCurve(const std::set<CurveId> &changedCurves);
     ...
     */
    std::map<CurveId, CurvePtr> flattenCurves;

    /**
     * Reference counts for each flattened Curve.
     */
    std::map<CurvePtr, int> flattenCurveCount;
};
