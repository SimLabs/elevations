...
public:
     ...
     */
    bool equals(std::set<CurveId> curveList);
     ...
     */
    bool equals(Area *a, std::set<CurveId> &visitedCurves, std::set<NodeId> &visitedNodes);
     ...
     */
    static bool clip(std::vector<CurvePart*> &cpaths, const box2d &clip, std::vector<CurvePart*> &result);
     ...
     */
    static bool isDirect(const std::vector<Vertex> &points, int start, int end);
...
private:
     ...
     */
    mutable std::vector<std::pair<CurvePtr, int> > curves;
     ...
     */
    bool build(const CurvePtr p, NodePtr start, NodePtr cur, const std::set<CurveId> &excludedCurves, std::set<CurvePtr> &visited, std::set<CurvePtr> &visitedr);
