...
public:
    ...
    struct Changes
    {
        std::list<AreaId> changedArea;

        /**
         * List of removed Curves.
         */
        std::set<CurveId> removedCurves;

        /**
         * List of added Curves.
         */
        std::set<CurveId> addedCurves;

        /**
         * List of removed areas.
         */
        std::set<AreaId> removedAreas;

        /**
         * List of added areas.
         */
        std::set<AreaId> addedAreas;
     ...
     */
    void getAreasFromCurves(const std::set<CurveId> &curves, std::set<AreaId> &areas);
     ...
     */
    void getPointsFromCurves(std::vector<CurveId> &curves,
            std::map<CurveId, int> orientations, std::vector<Vertex> &points);
     ...
     */
    virtual void load(const std::string &file, bool loadSubgraphs = true) = 0;
     ...
     */
    virtual void save(const std::string &file, bool saveAreas = true,
     ...
     */
    virtual CurvePtr addCurvePart(CurvePart &cp, std::set<CurveId> *addedCurves,
            bool setParent = true);
     ...
     */
    void addCurvePart(CurvePart &cp, std::set<CurveId> *addedCurves,
            std::set<CurveId> &visited, AreaPtr a);
     ...
     */
    bool buildArea(CurvePtr begin, std::set<CurveId> &excluded,
            std::vector<CurveId> &used, std::map<CurveId, int> &orientations,
            int orientation);
     ...
     */
    static void fitCubicCurve(std::vector<vec2d> points, std::vector<vec2d> &output, float error);
...
private:
     ...
     */
    std::map<vec2d, Node*, Cmp> *mapping;
     ...
     */
    static void followHalfCurve(NodePtr prev, NodePtr cur, std::vector<NodePtr> &result,
            std::set<CurvePtr> &visited, bool invert, bool useType, float *width);
     ...
     */
    static void followCurve(CurvePtr c, bool useType, std::set<CurvePtr> &visited,
            float *width, int *type, std::vector<NodePtr> &result);
