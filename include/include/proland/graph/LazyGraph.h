...
public:
    ...
    public:
        ...
        bool remove(ptr<T> t)
        {
            typename std::map<ptr<T> , typename std::list<ptr<T> >::iterator>::iterator i = unusedResources.find(t);
        ...
        {
            typename std::set<ptr<T> >::iterator i = find(changedResources.begin(), changedResources.end(), t);
            ...
            typename std::list<ptr<T> >::iterator j;
     ...
     */
    void load(const std::string &file, bool loadSubgraphs = 0);
...
protected:
     ...
     */
    std::map<NodeId, long int> getNodeOffsets() const;

    /**
     * Returns the list of offsets for each curve.
     * See LazyGraph description.
     */
    std::map<CurveId, long int> getCurveOffsets() const;

    /**
     * Returns the list of offsets for each area.
     * See LazyGraph description.
     */
    std::map<AreaId, long int> getAreaOffsets() const;
     ...
     **/
    std::map<NodeId, long int> nodeOffsets;

    /**
     * The offsets of each Curve in the input file.
     * Loaded in the #load() function.
     **/
    std::map<CurveId, long int> curveOffsets;

    /**
     * The offsets of each Area in the input file.
     * Loaded in the #load() function.
     **/
    std::map<AreaId, long int> areaOffsets;

    /**
     * The offsets of each subgraph in the input file.
     * Loaded in the #load() function.
     **/
    std::map<AreaId, long int> subgraphOffsets;
