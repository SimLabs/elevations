...
public:
     ...
     */
    void clip(const box2d &clip, std::vector<CurvePart*> &result) const;
     ...
     * @param result where the resulting curve parts must be stored.
     */
    static void clip(const std::vector<CurvePart*> &paths, const box2d &clip, std::vector<CurvePart*> &result);
