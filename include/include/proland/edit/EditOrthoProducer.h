...
public:
    ...
    EditOrthoProducer(ptr<TileCache> cache, ptr<TileProducer> residualTiles,
        ...
        ptr<Program> upsample, vec4f rootNoiseColor, vec4f noiseColor, std::vector<float> &noiseAmp, bool noiseHsv,
        float scale, int maxLevel, ptr<Module> edit, ptr<Program> brush, ptr<Program> compose,
        ptr<ResourceManager> manager, const std::string &terrain);
    ...
    virtual void edit(const std::vector<vec4d> &strokes);
...
protected:
     ...
     */
    void init(ptr<ResourceManager> manager, ptr<Texture2D> layerTexture, ptr<Module> edit, ptr<Program> brush, ptr<Program> compose, const std::string &terrain);
...
private:
     ...
     */
    std::map<GPUTileStorage::GPUSlot*, unsigned char*> backupedTiles;
     ...
     */
    void edit(ptr<TerrainQuad> q, const std::vector<vec4d> &strokes, const std::vector<box2f> &strokeBounds, int newStrokes);
