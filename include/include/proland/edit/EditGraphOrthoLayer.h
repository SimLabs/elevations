...
public:
    ...
    public:
         ...
         */
        EditGraphHandler(EditGraphOrthoLayer *e, ptr<ResourceManager> r, std::string t);
    ...
    public:
        ...
        {
            for(std::map<EditGraphOrthoLayer *, EditGraphOrthoLayer::EditGraphHandler*>::iterator i = handlers.begin(); i != handlers.end(); i++) {
                delete i->second;
            }
            handlers.clear();
        }
         ...
         */
        std::map<EditGraphOrthoLayer*, EditGraphOrthoLayer::EditGraphHandler*> handlers;
        ...
        void getTypeNames(std::vector<std::string> &e)
        {
            if ((int) handlers.size() == 0) {
                return;
            }
     ...
     */
    EditGraphOrthoLayer(const std::vector< ptr<GraphProducer> > &graphs, ptr<Program> layerProg, int displayLevel = 0, float tolerance = 8.0f / 192.0f, bool softEdition = true, double softEditionDelay = 100000.0, bool deform = false, std::string terrain = "", ptr<ResourceManager> manager = NULL);
     ...
     */
    std::vector< ptr<GraphProducer> > getGraphs();
     ...
     */
    bool select(TileCache::Tile *t, double x, double y, float tolerance, GraphPtr &graph, std::list<AreaId> &areas, CurvePtr &curve, int &segment, int &point);
     ...
     * @return true if something was selected.
     */
    bool findCurve(GraphPtr p, double x, double y, float tolerance, GraphPtr &graph, std::list<AreaId> &areas, CurvePtr &curve, int  &segment, int &point);
     ...
     * @return true if something was selected.
     */
    bool select(double x, double y, float tolerance, GraphPtr &graph, std::list<AreaId> &areas, CurvePtr &curve, int &segment, int &point);
     ...
     */
    virtual void getTypeNames(std::vector<std::string> &names);

    /**
     * List of points displayed on the Tile when drawing. They indicates current edition.
     */
    std::vector<vec2i> displayedPoints;
...
protected:
     ...
     * @param manager the ResourceManager used to create the scene.
     */
    virtual void init(const std::vector< ptr<GraphProducer> > &graphs, ptr<Program> layerProg, int displayLevel = 0, float tolerance = 8.0f / 192.0f, bool softEdition = true, double softEditionDelay = 100000.0, bool deform = false, std::string terrain = "", ptr<ResourceManager> manager = NULL);
     ...
     */
    std::vector< ptr<GraphProducer> > graphs;
     ...
     */
    std::set<TileCache::Tile::Id> usedTiles;
