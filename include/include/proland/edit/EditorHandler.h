...
{
public:
    Editor(std::string name="Editor", std::string group="defaultGroup", bool active = true);
     ...
     */
    virtual void edit(const std::vector<vec4d> &strokes) = 0;
...
public:
     ...
     * EditorHandler.
     */
    void setActive(std::string group, bool active);

    /**
     * True if any of the Editors that belong to 'group' is activated.
     */
    bool isActive(std::string group);
